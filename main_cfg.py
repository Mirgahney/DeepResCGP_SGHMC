...
def init_model(cfg, X, Y):
    ...
    if 'Res' in str(cfg.models.name):
        ...
        print('########### initializing PlainCGPNet #############')
...
def creat_task(cfg):
    model_name = f'{cfg.models.name}_fm_{cfg.models.params.feature_maps}_M{cfg.model.M}_K{cfg.model.kernel}_' \
                 ...
                 f'sghmc{cfg.train.sghmc_step}_train_sghmc_mode'
    save_dir = f'{cfg.top_dir}/{cfg.data.name}/{model_name}/0_{cfg.train.rep}'
    print(f'Save model at {save_dir}\n')
    writer = tf.compat.v1.summary.FileWriter(f'{save_dir}')
    return save_dir, writer

def train_model(cfg, model, Xvalid, Yvalid, Xtest, Ytest, writer, save_dir):
    mll_max = -np.inf
    best_iter = 0
    best_model = model
...
def train_model_sghmc(cfg, model, Xvalid, Yvalid, Xtest, Ytest, writer, save_dir):
    mll_max = -np.inf
    best_iter = 0
    best_model = model
    for i in tdqm(range(cfg.train.iterations), ascii=" .oO0", bar_format="{total_time}: {percentage:.0f}%|{bar}{r_bar}"):
...
def main(cfg):
    ...
    # train model
    train_model(cfg, model, Xvalid, Yvalid, Xtest, Ytest, writer, save_dir)
